...
#define DRV_NAME "miniaccel_edu_drv"
...
#define miniaccel_kzalloc_obj(ptr, flags) kzalloc(sizeof(*(ptr)), (flags))
...
static int miniaccel_begin_command(struct miniaccel_dev *mdev,
				   u32 expected_irq_status, u64 *submit_seqno)
{
	unsigned long flags;
	...
	mdev->submit_seqno++;
	if (submit_seqno)
		*submit_seqno = mdev->submit_seqno;
	mdev->expected_irq_status = expected_irq_status;
	mdev->in_flight = true;
	spin_unlock_irqrestore(&mdev->irq_lock, flags);
...
static void miniaccel_clear_in_flight(struct miniaccel_dev *mdev)
{
	unsigned long flags;
...
static long miniaccel_ioctl_run_sync_edu(struct miniaccel_dev *mdev,
					 struct miniaccel_run_sync *run)
{
	...
	ret = miniaccel_begin_command(mdev, EDU_IRQ_FACTORIAL, NULL);
	if (ret)
		goto unlock;
...
clear_in_flight:
	miniaccel_clear_in_flight(mdev);
				     ...
				     void __user *argp)
{
	...
	ret = miniaccel_ioctl_run_sync_edu(mdev, &run);
	if (ret)
		return ret;
...
static int miniaccel_open(struct inode *inode, struct file *file)
{
	...
	mfile = miniaccel_kzalloc_obj(mfile, GFP_KERNEL);
	if (!mfile)
		return -ENOMEM;
	mfile->mdev = mdev;
...
static irqreturn_t miniaccel_irq_handler(int irq, void *dev_id)
{
	...
	mdev->irq_count++;
	if ((status & mdev->expected_irq_status) && mdev->in_flight) {
...
static int miniaccel_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	mdev = miniaccel_kzalloc_obj(mdev, GFP_KERNEL);
	if (!mdev)
		return -ENOMEM;
	...
	dev_info(&pdev->dev,
		 "probe ok: EDU regs=%p version=0x%08x liveness=0x%08x\n",
...
static int __init miniaccel_drv_init(void)
{
	...
	miniaccel_class = class_create(THIS_MODULE, "miniaccel_edu");
...
MODULE_DESCRIPTION("PCI driver for QEMU EDU fallback (1234:11e8)");
MODULE_VERSION("0.1");
